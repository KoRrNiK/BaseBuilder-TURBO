...
public plugin_precache(){
	...
	spriteShar		=	precache_model(SHARSPRITE);
...
public plugin_init(){	
	...
	for(new i; i < sizeof(blockCommandAll); i++) register_clcmd(blockCommandAll[i],  "blockCommand")
...
public ham_TakeDamageEnt(ent, idinflictor, id, Float:damage, damagebits){
	...
	#if defined SCHROOM_ADDON
		if(!pev_valid(ent) || !is_user_alive(shroomPlayer))
			return HAM_IGNORED
...
public fw_PlayerPreThink(id){
	if( !is_user_alive(id) || !is_user_connected(id) || userFPS[id] >= 105)
		return FMRES_IGNORED
...
public globalHud(id){
	...
	if(userLogged[id] && !(userExtraFps[id] || userFPS[id] >= 150)){
		...
		//iLen += format(gText[iLen], sizeof(gText)-iLen-1, "|^tPostac - Lv: %d - XP: %0.1f / %0.1f^t^n", userLevel[id], userExp[id],needXp(id, userLevel[id]))
			...
			if(get_user_team(victim) == 1){
				if( userClass[victim] == class_HEALTH ){
					...
					for( new i = 1 ; i < maxPlayers;i ++ ){
						...
						if( !is_user_connected(i) || !is_user_alive(i) || get_user_team(i) != 1  || get_user_godmode(i) ||  i == attacker || i == victim)
							continue;
...
public classTakeDamage(victim, inflictor, attacker, &Float:damage, damagebits){
	...
	if(get_user_team(attacker) == 1){
		...
		if( userClass[attacker] == class_TERMINATOR){
			damage = 10.0;
		}
	}
...
public changeTeamIfSuggested(id){
	if( userSuggestTeam[id] == 0 || gameTime){
		return;
	}
...
public fw_EmitSound(id,channel,const sample[],Float:volume,Float:attn,flags,pitch){
	if (!is_user_connected(id) || get_user_team(id) != 1 || buildTime || prepTime || roundEnd)
		return FMRES_IGNORED;
...
public infoPlayer(id, target){
	...
	if(!(id == target))
		ColorChat(target, GREEN, "%s Gracz^x03 %s^x01 przeglada twoje statystyki!", PREFIXSAY, userName[id])
	
	show_motd(id, gText, userName[target])
	return PLUGIN_CONTINUE
}
